...
export const onRequestGet: PagesFunction<Env> = async (context) => {
  ...
  const posts = await Promise.all(files.filter(file => file.name.endsWith('.md')).map(async file => ({ filename: file.name, content: await (await fetch(file.download_url)).text() })));
